...
""")
    ...
    if install_ceos_utils:
        ...
        _shell_cmd(["sed", "-i", "'s/for i, data_arr_name in enumerate(rng.keys()):/"\
                    "for i, data_arr_name in enumerate(set(rng.keys()).intersection(set(data_arr_names))):/g'",
                    "./utils/data_cube_utilities/clean_mask.py"])
